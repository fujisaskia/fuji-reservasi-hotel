...
<div class="max-w-5xl mx-auto bg-white py-8 px-6 rounded-lg shadow-md border border-gray-200 text-sm md:text-xs">
    <h2 class="text-2xl pb-6 font-bold text-center mb-4 border-b">Pengguna</h2>

    <div class="flex flex-col md:flex-row items-start md:items-center justify-between space-y-3 md:space-y-0 mb-6">
        <!-- Form Search -->
        <form action="{{ route('users.index') }}" method="GET" class="flex order-1 w-full md:w-auto gap-2">
            <input 
                type="search" 
                name="search" 
        ...
        <!-- Button Tambah -->
        <div class="flex w-full md:w-auto justify-end order-2">
            <a href="/users/create">
                <button class="flex items-center text-white bg-green-500 hover:bg-green-600 focus:bg-green-500 px-4 py-3 lg:py-2 rounded-lg w-full md:w-auto">
                    <i class="fa-solid fa-user-plus"></i>
                    <p class="ml-2">Tambah <span class="hidden md:flex">Pengguna</span></p>
                </button>
            </a>
        </div>
    </div>
            ...
            <table class="min-w-full bg-white border border-gray-200">
                <thead>
                    <tr class="bg-rose-100">
                        ...
                        <th class="py-3 px-4 border-b border-gray-200 text-left font-semibold text-gray-600">Nama</th>
                        <th class="py-3 px-4 border-b border-gray-200 text-left font-semibold text-gray-600">Email</th>
                        <th class="py-3 px-4 border-b border-gray-200 text-left font-semibold text-gray-600">Telepon</th>
                        <th class="py-3 px-4 border-b border-gray-200 text-left font-semibold text-gray-600">Peran</th>
                        <th class="py-3 px-4 border-b border-gray-200 text-center font-semibold text-gray-600">Aksi</th>
                ...
                <tbody>
                    @foreach ($users as $index => $user)                    
                        <tr class="hover:bg-gray-100 group">
                            ...
                            <td class="py-3 px-4 border-b border-gray-200 text-gray-600">{{ $user->full_name }}</td>
                            <td class="py-3 px-4 border-b border-gray-200 text-gray-600">{{ $user->email }}</td>
                            <td class="py-3 px-4 border-b border-gray-200 text-gray-600">{{ $user->phone_number ?? 'nomor tidak tersedia' }}</td>
                            <td class="py-3 px-4 border-b border-gray-200 text-gray-600 bg-slate-50 group-hover:bg-white">{{ $user->role }}</td>
